...
# The order of point selection:
	# Main points
		...
		#6. Eroe of the auricle of the left atrium

	# Auxiliary points
	#1. Front wall
	#2. Back wall
	#3. The Eroe of the left superior pulmonary vein
	#4. The Eroe of the right superior pulmonary vein

###### NOTION : THE NAME OF THE MESH IS CHANGED MANUALLY #######
...
path = '/home/nastasiya/jove/mesh/Test/Endo/'
#path = '/home/nastasiya/jove/mesh/Test/Epi/'
meshname = 'Test_endo_hole.stl'
#meshname = 'Test_epi_hole.stl'

# Output filenames for the two point sets
main_outname = 'main_points_to_par.txt'
...
pl.enable_surface_picking(show_message="Pick 10 points in order:\n"
                 "1-6: Main anatomical landmarks\n"
                 "7-10: Auxiliary landmarks\n"
...
main_points = picked_points[:6]
aux_points = picked_points[6:10]

# Save main points to CSV/TXT
main_path = os.path.join(path, main_outname)
pd.DataFrame(main_points).to_csv(main_path, header=None, index=False, sep=',')
print(f" Successfully saved {len(main_points)} main points to '{main_outname}'")

# Save auxiliary points to CSV/TXT
