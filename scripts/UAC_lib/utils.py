...
from UAC_lib.element import Element
from UAC_lib.line import Line


# Число точек на элемент
eleminfo = {"Ln": 2, "Tr": 3, "Qd": 4, "Tt": 4, "Py": 5, "Pr": 6, "Hx": 8}
# Перевод числа точек carp в элементы поверхности
carpsurf = {2: "Ln", 3: "Tr", 4: "Qd"}
# Перевод CARP в VTK элементы
...
# Загрузка точек регионов (ушко, вены)
...
# Вычисляет квадраты расстояний между заданной точкой и всеми точками в массиве
'''
Важный нюанс: функция возвращает не индекс ближайшей точки, 
а массив квадратов расстояний до всех точек. Для нахождения ближайшей точки 
нужно использовать np.argmin() на результате.
'''
def closest_node(node, nodes):
    deltas = nodes - node
    #сумма эйнштейна - Вычисление квадратов расстояний
    # result[i] = Σ_j (deltas[i,j] * deltas[i,j])
    dist_2 = np.einsum("ij,ij->i", deltas, deltas)
    return dist_2

# Находит граничные ребра сетки — то есть ребра, 
# которые принадлежат только одному элементу
# и создает словарь элементов по каждому ребру, 
# а также возвращает набор узлов на границе
def find_edges(elems, pts):
    if verbose >= 1:
        print("Finding border edges")
    bedges = dict()
    for e in elems:
        ees = e.edges()  # Получаем все ребра элемента
        for edge in ees: 
            if edge in bedges:
                del bedges[edge] # Ребро уже есть - значит оно внутреннее
            else:
                bedges[edge] = e.index # Ребро новое - добавляем
    edgenode = set()
    for b in bedges:
        edgenode.add(b.n[0]) # Добавляем первый узел ребра
        edgenode.add(b.n[1]) # Добавляем второй узел ребра
...
"""Находит узлы миокарда в качестве таковых в самом большом связанном списке ребер"""
'''
Функция анализирует граничные ребра сетки, объединяет их в непрерывные линии 
и выбирает одну конкретную линию (самую длинную) 
'''
def find_nodes(elems, pts, no=-1, ascending=False):
    border, _ = find_edges(elems, pts) #получены все граничные рёбра
    lines = coalesce(border) #объединяет отдельные ребра в непрерывные линии
    ...
    if no == -1:
        # выбирает самую длинную линию (по умолчанию)
        index = np.argmax(line_size)
    else:
        # выбирает линию с определенным порядковым номером после сортировки
...
# Алгоритм поиска ближайших соответствий между двумя наборами точек
# Сопоставляет найденные PV с ожидаемыми позициями
...
def read_array_igb(igbfile):
    ...
    with open(igbfile, mode="rb") as file:
        ...
        nnode = word[0] * word[1] * word[2] # Вычисляет общее количество узлов сетки как произведение размеров по трем измерениям.

        size = os.path.getsize(igbfile) // 4 // nnode # Количество временных шагов в файле
        data = np.empty((size, nnode), float)
        for i in range(size):
            data[i] = struct.unpack("f" * nnode, file.read(4 * nnode)) #преобразует байты в nnode значений float
    return data
...
def add_celldata_array(polydata, arrayname, initval=0):
    """функция добавляет новый массив данных для ячеек (элементов) в VTK полидату"""
...
def add_labels(surface, reg_name, src_regs):
    """
    Добавляет новый массив к поверхности и помечает маркером (значение по умолчанию 1.0).
    Все точки в пределах круговой области вокруг центра и с заданным радиусом.
    Немаркированные точки помечаются значением unmarked (по умолчанию 0.0).
    """
...
def boundary_markers(index1, index2, tot_pts):
    """Calculates markers 1-4 in UAC_lib paper"""
